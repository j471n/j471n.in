export function showHoverAnimation(e, isDarkMode) {
  ...
  if (isDarkMode) {
    ...
    e.target.style.background = `radial-gradient(circle at ${x}px ${y}px , rgba(0,0,0,0.2),rgba(0,0,0,0) )`;
    e.target.style.borderImage = `radial-gradient(20% 75% at ${x}px ${y}px ,rgba(0,0,0,0.7),rgba(0,0,0,0.1) ) 1 / 1px / 0px stretch `;
  }
}
