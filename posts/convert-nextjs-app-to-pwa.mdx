...
```bash npm i next-pwa # npm yarn add next-pwa # yarn ```

After installation go to your next `next.config.js` as update it as follows -

<CodeTitle title="next.config.js" />
```js
...
<CodeTitle title="directory" />
``` ├── public | ├── apple-touch-icon.png | ├── favicon.ico | ├── icons | | ├── icon-192x192.png
| | ├── icon-256x256.png | | ├── icon-384x384.png | | ├── icon-512x512.png | | └──
maskable.png | ├── manifest.json ```
